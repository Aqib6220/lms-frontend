...
export const getPendingCourses = createAsyncThunk(
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch pending courses"
      );
    }
  }
);
...
export const fetchTrainerCourses = createAsyncThunk(
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch trainer courses"
      );
    }
  }
);
...
export const updateCourseApproval = createAsyncThunk(
  ...
  async (
    { courseId, status, rejectionReason },
    ...
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to update course approval"
      );
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteCourse.fulfilled, (state, action) => {
        state.trainerCourses = state.trainerCourses.filter(
      ...
      .addCase(updateCourse.fulfilled, (state, action) => {
        ...
        if (
          ...
          state.selectedCourse._id === action.payload._id
        ) {
